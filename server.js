...
async function main(){
    ...
    app.get('/profiles/:username/:tab', async (req, res) => {
        ...
        if (tab === 'posts') {
            userPosts = await user_posts.find({'createdBy.username': viewedUser }).lean().exec();
        } else if (tab === 'comments') {
            userPosts = null // replace with schema for comments
        } else if (tab === 'upvotes') {
            userPosts = await user_posts.find({}).lean().exec();
        }
        res.render("profiles", {
            layout: false,
            title: "UserProfile",
